...
describe('c-radio-group', () => {
  ...
  it('value change radio', () => {
    ...
    return Promise.resolve()
      .then(() => {
        expect(shadowQuerySelectorAll(element, 'input')[0].checked).toBe(true);
        expect(shadowQuerySelectorAll(element, 'input')[1].checked).toBe(false);
        element.value = 'option2';
      })
      .then(() => {
        expect(shadowQuerySelectorAll(element, 'input')[0].checked).toBe(false);
        expect(shadowQuerySelectorAll(element, 'input')[1].checked).toBe(true);
      });
  });

...
describe('record edit form', () => {
      ...
      function loadHandler() {
        ...
        element.addEventListener('load', () => {
          try {
            ...
            expect(data.record.fields.Name.value).toEqual('Darth Vader');
          } catch (e) {
            reject(e);
          }
          resolve();
  ...
  it('submits dirty fields', () => {
    ...
    return new Promise((resolve, reject) => {
      element.addEventListener('load', () => {
        ...
        const form = shadowQuerySelector(element, 'lightning-record-edit-form');
        ...
        form.addEventListener('success', () => {
          try {
            ...
            expect(window.LAST_SAVED_RECORD.fields.Name).toEqual('Banana');
          } catch (err) {
            reject(err);
          }
          resolve();
  ...
  it('sends recordTypeId with fields when specified', () => {
    ...
    return new Promise((resolve, reject) => {
      element.addEventListener('load', () => {
        ...
        form.addEventListener('success', () => {
          try {
            expect(window.LAST_SAVED_RECORD.fields.RecordTypeId).toEqual(
              'blahblah'
            );
          } catch (err) {
            reject(err);
  ...
  it('creates a record', () => {
    ...
    return new Promise((resolve, reject) => {
      element.addEventListener('load', () => {
        ...
        form.addEventListener('success', () => {
          try {
            expect(window.LAST_SAVED_RECORD.fields.Id).toBeUndefined();
            expect(window.LAST_SAVED_RECORD.fields.Name).toEqual('Banana');
          } catch (err) {
            reject(err);
          }
        ...
        if (inputField) {
          try {
            ...
            expect(data.Weakness__c).toEqual(
              picklistRepresentation.Bad_Guy__c[DEFAULT_RECORD_TYPE_ID]
                .Weakness__c
            );
          } catch (e) {
            reject(e);
        ...
        if (inputField) {
          try {
            ...
            expect(data.Country__c).toEqual(
              picklistRepresentation.Bad_Guy__c['012xx0000000011'].Country__c
            );
          } catch (e) {
            reject(e);
          }
    ...
    return new Promise((resolve, reject) => {
      element.addEventListener('load', () => {
        ...
        form.addEventListener('error', () => {
          const messages = shadowQuerySelector(element, 'lightning-messages');

          Promise.resolve().then(() => {
            try {
              expect(messages.error.message).toEqual('Error happened');
            } catch (err) {
              return reject(err);
            }
            return resolve();
  ...
  it('wires label alignment for auto density and reacts on changing density', () => {
    ...
    return new Promise((resolve, reject) => {
      element.addEventListener('load', () => {
        ...
        if (outputField) {
          try {
            expect(densityUtils.resetResizeObserver).toHaveBeenCalledTimes(1);
          } catch (e) {
            reject(e);
          }
          resolve();
      ...
      return Promise.resolve().then(() => {
        expect(densityUtils.resetResizeObserver).toHaveBeenCalledTimes(2);
      });
    });
  });
  ...
  it('wires label alignment for comfy density in edit mode', () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      const inputField = shadowQuerySelector(element, 'lightning-input-field');

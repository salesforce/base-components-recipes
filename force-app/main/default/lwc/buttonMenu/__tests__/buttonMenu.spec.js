...
describe('c-button-menu-defaults', () => {
...
describe('c-button-menu-onselect', () => {
  it('should fire onselect event when click on a menu item', () => {
    ...
    return Promise.resolve().then(() => {
      ...
      return Promise.resolve().then(() => {
        shadowQuerySelector(
          shadowQuerySelectorAll(buttonMenu, 'lightning-menu-item')[2],
          'a'
        ).click();
        expect(callback.mock.calls).toHaveLength(1);
        const selectedMenu = callback.mock.calls[0][0].selectEvent.detail.value;
...
describe('c-button-menu-variant-attr', () => {
  [
    ...
    it(`button class attribute should reflect variant=${variant}`, () => {
      ...
      return Promise.resolve().then(() => {
        const classes = shadowQuerySelector(buttonMenu, 'button').className;
...
describe('c-button-menu-menu-alignment-attr', () => {
  ...
  [
    ...
    it(`should have correct classes when menu-alignment=${menuAlignment.value}`, () => {
      ...
      return Promise.resolve().then(() => {
        const dropdown = shadowQuerySelector(buttonMenu, menuContainerSelector);

        expect(
          dropdown.classList.contains(`slds-dropdown_${menuAlignment.class}`)
        ).toBe(true);
      });
    });
  });
...
describe('c-button-menu-disabled-attr', () => {
  it('should show disabled', () => {
    ...
    return Promise.resolve().then(() => {
      expect(
        shadowQuerySelector(buttonMenu, 'button').hasAttribute('disabled')
      ).toBe(true);
    });
  });
});
